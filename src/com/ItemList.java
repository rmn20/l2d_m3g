	...
	public final void draw(Graphics g, int x1, int y1, int x2, int y2) {
		...
		for(; var8 < this.items.length; ++var8) {
			if(var7 >= 0) {
				...
				if(var8 == this.index) {
					int var12 = x2 / 2 - this.font.widthOf(var11) / 2;
					int var9 = this.font.widthOf(var11);
					int var10 = this.font.height();
					g.setColor(16711680);
					g.drawRect(var12 - 1, var7 - 1, var9 + 1, var10 + 1); // Красная рамка вокруг выбранного пункта меню
				}

				this.font.drawString(g, var11, x2 / 2 - this.font.widthOf(var11) / 2, var7, 0);
				var7 += x1;
			}
		}
